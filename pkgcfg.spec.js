var log = require('ulog')('pkgcfg:spec')
...
describe('pkgcfg', function(){
	...
	it('calls transform functions associated to tags that it encounters during parsing', function(){
		log.info('$test{} => calls $test(root, parents, node)')
		var called = false
		function $test(root, parents, node) {
			called = true
		}
	...
	it('calls transform functions with names containing a dash', function(){
		log.info('$test-with-dash{} => calls $test(root, parents, node)')
		var called = false
		function $test(root, parents, node) {called = true}
	...
	it('calls transform functions with names containing a colon', function(){
		log.info('$test:with:colon{} => calls $test(root, parents, node)')
		var called = false
		function $test(root, parents, node) {called = true}
	...
	it('calls transform functions with names containing a dot', function(){
		log.info('$test.with.dot{} => calls $test(root, parents, node)')
		var called = false
		function $test(root, parents, node) {called = true}
	...
	it('calls transform functions with names containing a quote', function(){
		log.info("$test'with'quote{} => calls $test(root, parents, node)")
		var called = false
		function $test(root, parents, node) {called = true}
	...
	it('yields undefined for matched tags returning undefined', function(){
		log.info('$test{} => calls $test(root, parents, node), yields undefined');
		var called = false
		function $test(root, parents, node) {called = true}
		pkgcfg.registry.register('$test', $test)
		try {
	...
	it('yields \'null\' for matched tags returning null', function(){
		log.info('$test{} => calls $test(root, parents, node), yields null')
		var called = false
		function $test(root, parents, node) {called = true; return null}
		pkgcfg.registry.register('$test', $test)
		try {
			var pkg = pkgcfg()
			expect(called).to.equal(true)
	...
	it('calls transform functions with a string argument', function(){
		log.info('$test-with-arg{some arg} => calls $test(root, parents, node, \'some arg\')');
		var called = false
		function $test(root, parents, node, arg) {
	...
	it('calls transform functions with an object argument', function(){
		log.info('$test-with-arg-object{{\'some\':\'object\'}} => calls $test(root, parents, node, {some:\'object\'})');
		var called = false
		function $test(root, parents, node, arg) {
	...
	it('calls transform functions with array arguments', function(){
		log.info('$test-with-arg-array{[\'some\',\'array\']} => calls $test(root, parents, node, [\'some\',\'array\'])')
		var called = false
		function $test(root, parents, node, arg) {
	...
	it('calls transform functions with arguments list', function(){
		log.info('$test-with-arg-list{(\'some\',\'list\')} => calls $test(root, parents, node, \'some\', \'list\')')
		var called = false
		function $test(root, parents, node, arg0, arg1) {
	...
	it('calls transform functions with quoted unbalanced close markers', function(){
		log.info('$test-with-unbalanced-close{\'unbalanced } close\'} => calls $test(root, parents, node, \'unbalanced } close\')')
		var called = false
		function $test(root, parents, node, arg) {
	...
	describe('registry', function(){
		function $hello(root, parents, node, text) {
		...
		describe('register', function(){
			it('adds the given package transform function under the given tag', function(){
				...
				var pkg = pkgcfg()
				expect(pkg.test.register).to.equal('Hello, John!')
			})
		})
		...
		it('contains a built-in transform {$ ref} / ${ref}', function(){
			expect(pkgcfg.registry.getTransformTags().indexOf('$')).not.to.equal(-1)
		})
	})
})
...
describe('built-in transform', function(){
	describe('${ref}', function(){
		...
		it('returns the referenced nested primitive value', function(){
			log.info('${repository.type} => \'git\'')
			expect(pkg.test['nested-primitive']).to.equal(pkg.repository.type)
		})
		...
		it('returns the referenced nested object', function(){
			log.info('${test.data} => {array:[\'one config to ..\', ..], ..}')
		...
		it('returns the referenced array', function(){
			log.info('${keywords} => [\'package.json\', ...]')
		...
		it('returns the text unmodified if it failed to resolve', function(){
			log.info('${unresolved.transforms.return.unmodified.text} => \'${unresolved.transforms.return.unmodified.text}\'')
			expect(pkg.test.unresolved).to.be.a('string')
			expect(pkg.test.unresolved).to.equal('${unresolved.transforms.return.unmodified.text}')
		})

		// "array-length": "${test.data.array.length}",
		it('returns the referenced array length', function(){
			log.info('${test.data.array.length} => 3')
		...
		it('returns the referenced nth item of the array', function(){
			log.info('${test.data.array.2} => pkgcfg')
		...
		it('resolves references containing a dash', function(){
			log.info('${test.data.with-dash} => ok')
		...
		it('resolves references containing a colon', function(){
			log.info('${test.data.with:colon} => ok')
		...
		it('resolves references containing a dot', function(){
			log.info('${[\'test\',\'data\',\'with.dot\']} => ok')
		...
		it('resolves references containing a quote', function(){
			log.info('${[\'test\',\'data\',\'with\'\'quote\']} => ok')
		...
		it('resolves indirect references', function(){
			log.info('${test.primitive} => pkgcfg')
			expect(pkg.test['indirect']).to.be.a('string')
			expect(pkg.test['indirect']).to.equal('pkgcfg')
		})

		// "tag-within-tag": "$test{${name}}"
		it('resolves references within other tags', function(done){
			log.info('$echo{${name}} => pkgcfg')
			var called = false
			function $echo(root, parents, node, arg) {
				var expected = 'pkgcfg'
